...
// 安全的 LocalStorage 讀取與寫入 (防止 Vercel 編譯時因伺服器無 window 物件而崩潰)
...
// 初次載入：執行智能保留期限 (7天) 及清理邏輯 (移出組件以避免 React 依賴項警告)
const initializeNewsData = () => {
    const localNews = safeGetLocal('react_news_v3', []);
    const localBookmarks = safeGetLocal('react_bookmarks_v3', []);
    const localUnbookmarked = safeGetLocal('react_unbookmarked_v3', {});
    ...
    if (cleaned.length !== localNews.length) {
        safeSetLocal('react_news_v3', cleaned);
    }
    return cleaned;
};
...
export default function App() {
    // 狀態管理 (React State)
    const [categories, setCategories] = useState(() => safeGetLocal('react_cats_v3', [...defaultCategories]));
    const [newsData, setNewsData] = useState(initializeNewsData);
    const [readArticles, setReadArticles] = useState(() => safeGetLocal('react_read_v3', []));
    const [bookmarks, setBookmarks] = useState(() => safeGetLocal('react_bookmarks_v3', []));
    const [unbookmarkedTracker, setUnbookmarkedTracker] = useState(() => safeGetLocal('react_unbookmarked_v3', {}));
    ...
    // 儲存狀態至 LocalStorage
    useEffect(() => { safeSetLocal('react_news_v3', newsData); }, [newsData]);
    useEffect(() => { safeSetLocal('react_bookmarks_v3', bookmarks); }, [bookmarks]);
    useEffect(() => { safeSetLocal('react_unbookmarked_v3', unbookmarkedTracker); }, [unbookmarkedTracker]);
    useEffect(() => { safeSetLocal('react_read_v3', readArticles); }, [readArticles]);
        ...
        if (isSettingsOpen && listRef.current && typeof window !== 'undefined' && window.Sortable) {
            ...
            sortableRef.current = new window.Sortable(listRef.current, {
                ...
                onEnd: () => {
                    ...
                    safeSetLocal('react_cats_v3', newOrder);
                }
            });
        }
    }, [isSettingsOpen]);
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setIsSettingsOpen(false)}>
                    <div className="bg-[#1e293b] w-11/12 max-w-[400px] rounded-2xl flex flex-col shadow-2xl border border-slate-600" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-4 border-t border-slate-700 flex justify-between bg-[#1e293b] rounded-b-2xl">
                            <button onClick={() => { setCategories([...defaultCategories]); safeSetLocal('react_cats_v3', defaultCategories); }} className="text-xs text-slate-400 hover:text-white px-3 py-2">恢復預設</button>
